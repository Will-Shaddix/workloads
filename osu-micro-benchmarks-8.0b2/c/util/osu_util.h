...
#define MALLOC_CHECK_AND_RET(ptr, size, rank)                                  \
    do {                                                                       \
        ptr = malloc(size);                                                    \
        if (NULL == ptr) {                                                     \
            fprintf(stderr, "Failed to Malloc %lu bytes [rank %d]\n", size,    \
                    rank);                                                     \
            return ENOMEM;                                                     \
        }                                                                      \
    } while (0)
...
#ifndef TIGHT_FIELD_WIDTH
#define TIGHT_FIELD_WIDTH 12
#endif
...
#define ROCM_CHECK(stmt)                                                       \
    do {                                                                       \
        hipError_t _errno = (stmt);                                            \
        if (0 != _errno) {                                                     \
            fprintf(stderr, "[%s:%d] ROCM call '%s' failed with %d: %s \n",    \
                    __FILE__, __LINE__, #stmt, _errno,                         \
                    hipGetErrorString(_errno));                                \
            exit(EXIT_FAILURE);                                                \
        }                                                                      \
        assert(hipSuccess == _errno);                                          \
    } while (0)
#endif
...
#define OMB_IS_RECVBUF                  0
#define OMB_IS_SENDBUF                  1
#define MAX_MESSAGE_SIZE                (1 << 22)
#define MAX_MSG_SIZE_PT2PT              (1 << 20)
#define MAX_MSG_SIZE_COLL               (1 << 20)
#define MIN_MESSAGE_SIZE                1
#define MESSAGE_SIZE_INCR               2
#define MAX_ITERATION_SIZES             10
#define MAX_SKIP_SIZES                  10
...
enum test_domain { STARTUP, ONE_SIDED, PT2PT, COLLECTIVE, OSHM, UPC, UPCXX };
...
struct options_t {
    ...
    // legacy, until every benchmark is updated
    size_t iterations;
    size_t iterations_large;
    // new
    size_t iteration_sizes[MAX_ITERATION_SIZES];
    size_t iteration_counts[MAX_ITERATION_SIZES];
    int num_iteration_sizes;

    // legacy, until every benchmark is updated
    size_t skip;
    size_t skip_large;
    // new
    size_t skip_sizes[MAX_SKIP_SIZES];
    size_t skip_counts[MAX_SKIP_SIZES];
    int num_skip_sizes;
...
extern char const *not_launched_message;
