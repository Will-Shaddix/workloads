...
#ifndef _OSU_UTIL_MPI_H_
#define _OSU_UTIL_MPI_H_
...
#define MPI_CHECK(stmt)                                                        \
    do {                                                                       \
        int _errno = (stmt);                                                   \
        if (MPI_SUCCESS != _errno) {                                           \
            fprintf(stderr, "[%s:%d] MPI call failed with %d \n", __FILE__,    \
                    __LINE__, _errno);                                         \
            exit(EXIT_FAILURE);                                                \
        }                                                                      \
        assert(MPI_SUCCESS == _errno);                                         \
    } while (0)
...
#define DOUBLE_VALIDATION_MULTIPLIER (float)2.0
 ...
 * Memory Management
 */
typedef struct omb_buffer_sizes {
    size_t sendbuf_size;
    size_t recvbuf_size;
} omb_buffer_sizes_t;
...
extern MPI_T_cvar_handle dynamic_tuning_cvar;
#define OMB_PAUSE_DYNAMIC_TUNING                                               \
    do {                                                                       \
        if (dynamic_tuning_cvar) {                                             \
            MPI_T_cvar_write(dynamic_tuning_cvar, "0");                        \
        }                                                                      \
    } while (0)

#define OMB_START_DYNAMIC_TUNING                                               \
    do {                                                                       \
        if (dynamic_tuning_cvar) {                                             \
            MPI_T_cvar_write(dynamic_tuning_cvar, "1");                        \
        }                                                                      \
    } while (0)
