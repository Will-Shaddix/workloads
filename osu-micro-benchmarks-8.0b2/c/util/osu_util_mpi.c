...
MPI_T_cvar_handle dynamic_tuning_cvar = NULL;
...
void print_help_message(int rank)
{
    ...
    if (accel_enabled && (options.domain == PT2PT)) {
        fprintf(stdout, "Usage: %s [options] [SRC DST]\n\n", benchmark_name);
        fprintf(
            stdout,
            "SRC and DST are buffer types for the source and destination\n");
        if (options.subtype == MBW_MR) {
            ...
            fprintf(stdout, "SRC and DST may be `D', `H', 'MD'or 'MH' which "
                            "specifies whether\n"
                            "the buffer is allocated on the accelerator device "
                            "memory, host\n"
                            "memory or using CUDA Unified Memory allocated on "
                            "device or host "
...
void print_only_header(int rank)
{
    ...
    if (TYPE_BANDWIDTH == options.type || TYPE_BI_BANDWIDTH == options.type) {
        fprintf(stdout, "%*s", FIELD_WIDTH, "Bandwidth (MB/s)");
        if (options.print_rate) {
            fprintf(stdout, "%*s", FIELD_WIDTH, "Messages/s");
        }
    } else {
...
int omb_find_sep_index(int argc, char **argv)
{
    int i;
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--") == 0) {
...
static bool omb_requires_thread_multiple(char *path)
{
    int i;
    char *name, *last_slash;
    static const char *mt_tests[] = {"latency_mt", "osu_latency_mt", NULL};

    if (path) {
        /* Returns pointer to last occurence of / */
        last_slash = strrchr(path, '/');
        name = last_slash ? (last_slash + 1) : path;

        for (i = 0; mt_tests[i]; ++i) {
            if (strcmp(name, mt_tests[i]) == 0) {
                return true;
            }
        }
    }
...
static void omb_set_session_thread_level(const char *level)
{
#ifdef _ENABLE_MPI4_
    if (1 == options.omb_enable_session) {
        MPI_Info_set(options.session_info, "thread_level", level);
    }
#endif
}

static void omb_set_thread_level_from_args(int argc, char **argv)
{
    int sep, i;

    /* Executable might require thread_multiple */
    if (omb_requires_thread_multiple(argv[0])) {
        omb_set_session_thread_level("MPI_THREAD_MULTIPLE");
        options.thread_level = MPI_THREAD_MULTIPLE;
        return;
    }

    /* Launched test might require thread_multiple */
    sep = omb_find_sep_index(argc, argv);
    if (SEPARATOR_NONE != sep) {
        for (i = sep + 1; i < argc; ++i) {
            if (omb_requires_thread_multiple(argv[i])) {
                omb_set_session_thread_level("MPI_THREAD_MULTIPLE");
                options.thread_level = MPI_THREAD_MULTIPLE;
                return;
            }
    ...
    omb_set_session_thread_level("MPI_THREAD_SINGLE");
    options.thread_level = MPI_THREAD_SINGLE;
    return;
}

omb_mpi_init_data omb_mpi_init(int *argc, char ***argv)
{
    int cvar_index, count, err, my_rank;
    int actual_thread_level;
    omb_mpi_init_data init_struct;
#ifdef _ENABLE_MPI4_
    MPI_Group wgroup = MPI_GROUP_NULL;
    init_struct.omb_shandle = MPI_SESSION_NULL;
#endif
    omb_set_thread_level_from_args(*argc, *argv);
    ...
    if (1 == options.omb_enable_session) {
#ifdef _ENABLE_MPI4_
        {
            MPI_CHECK(MPI_Session_init(options.session_info, MPI_ERRORS_RETURN,
        ...
        MPI_CHECK(MPI_Init_thread(argc, argv, options.thread_level,
                                  &actual_thread_level));

        MPI_CHECK(MPI_Comm_rank(MPI_COMM_WORLD, &my_rank));

        if (actual_thread_level != options.thread_level) {
            if (!my_rank) {
                fprintf(stderr,
                        "%s%sMPI_Init_thread returned '%d', expected '%d'!%s\n",
                        OMB_ERR_BOLD, OMB_ERR_RED, actual_thread_level,
                        options.thread_level, OMB_ERR_RESET);
                fflush(stderr);
            }
            exit(EXIT_FAILURE);
        } else if (MPI_THREAD_MULTIPLE == actual_thread_level && !my_rank) {
            fprintf(stderr,
                    "\n%s%sMPI_THREAD_MULTIPLE ENABLED, EXPECT DIMINISHED "
                    "PERFORMANCE!%s\n",
                    OMB_ERR_BOLD, OMB_ERR_RED, OMB_ERR_RESET);
            fflush(stderr);
        }
    ...
    MPI_CHECK(MPI_T_init_thread(options.thread_level, &actual_thread_level));
    err = MPI_T_cvar_get_index("MPIR_CVAR_ENABLE_DYNAMIC_TUNING", &cvar_index);

    if (MPI_SUCCESS == err) {
        MPI_CHECK(MPI_T_cvar_handle_alloc(cvar_index, NULL,
                                          &dynamic_tuning_cvar, &count));
        assert(NULL != dynamic_tuning_cvar);
    }
...
{
    if (dynamic_tuning_cvar) {
        MPI_CHECK(MPI_T_cvar_handle_free(&dynamic_tuning_cvar));
    }
    MPI_T_finalize();
    if (1 == options.omb_enable_session) {
#ifdef _ENABLE_MPI4_
...
void omb_assign_element_for_validation(void *buf, int pos, int val,
                                       MPI_Datatype dtype)
{
    if (MPI_CHAR == dtype || MPI_SIGNED_CHAR == dtype) {
        ((char *)buf)[pos] =
                             ...
                             int iter, enum accel_type type, MPI_Datatype dtype)
{
    ...
    if (is_send_buf) {
        for (i = 0; i < num_elements; i++) {
            j = (i % 100);
            val = (j + 1) * (iter + 1);
            omb_assign_element_for_validation(temp_buffer, i, val, dtype);
        }
    } else {
        for (i = 0; i < num_elements; i++) {
            omb_assign_element_for_validation(temp_buffer, i, 0, dtype);
        }
    }
    switch (type) {
        case NONE:
        ...
        for (i = 0; i < num_procs; i++) {
            for (j = 0; j < num_elements; j++) {
                ...
                if (1 == options.omb_enable_mpi_in_place &&
                    ...
                    omb_assign_element_for_validation(
                        temp_buffer,
                        (id % com_size) * num_elements + (i * num_elements + j),
                        val, dtype);
                } else {
                    omb_assign_element_for_validation(
                        temp_buffer, i * num_elements + j, val, dtype);
                }
            }
        }
    } else {
...
#pragma acc parallel loop deviceptr(p)
            ...
            for (i = 0; i < num_elements; i++) {
                ...
                omb_assign_element_for_validation(expected_buffer, i, val,
                                                  dtype);
            }
            if (dtype == MPI_FLOAT) {
                for (i = 0; i < num_elements; i++) {
                ...
                for (i = 0; i < num_elements; i++) {
                    j = (i % 100);
                    if (abs(((double *)temp_r_buf)[i] -
                            ((double *)expected_buffer)[i]) > ERROR_DELTA) {
                        error = 1;
                        break;
                    }
                }
            ...
            if (error && options.log_validation) {
...
void init_nbhr_validation_buffers(void *s_buf, void *r_buf, int indegree,
                                  int *sources, int outdegree,
                                  int *destinations, size_t size,
                                  ...
                                  MPI_Datatype dtype)
{
    ...
    for (i = 0; i < send_numprocs; i++) {
        ...
        for (l = 0; l < omb_get_num_elements(size, dtype); l++) {
            ...
            omb_assign_element_for_validation(
                temp_s_buf, i * omb_get_num_elements(size, dtype) + l, res,
                dtype);
        }
    }
    for (l = 0; l < omb_get_num_elements(size, dtype) * recv_numprocs; l++) {
        omb_assign_element_for_validation(temp_r_buf, l, 0, dtype);
    }
    switch (type) {
        case NONE:
                       ...
                       enum accel_type type, MPI_Datatype dtype)
{
    ...
    for (i = 0; i < num_elements; i++) {
        j = (i % 100);
        val = (j + 1) * (iter + 1) * num_procs;
        omb_assign_element_for_validation(expected_buffer, i, val, dtype);
    }
    if (dtype == MPI_FLOAT) {
        for (i = 0; i < num_elements; i++) {
            j = (i % 100);
                        ...
                        enum accel_type type, int itr, MPI_Datatype dtype)
{
    ...
    for (i = 0; i < value2; i++) {
        for (j = 0; j < num_elements; j++) {
            ...
            omb_assign_element_for_validation(expected_buffer,
                                              i * num_elements + j, val, dtype);
        }
    }
    if (dtype == MPI_FLOAT) {
        for (i = 0; i < num_elements; i++) {
                    ...
                    size_t num_elements, MPI_Datatype dtype, int itr)
{
    ...
    } else if (MPI_SIGNED_CHAR == dtype) {
        fprintf(log_file_fp, "Datatype: MPI_SIGNED_CHAR\n");
    } else {
    ...
    } else if (MPI_CHAR == dtype || MPI_SIGNED_CHAR == dtype) {
...
#ifdef _ENABLE_OPENACC_
        case OPENACC:
            if (NULL == (*buffer = acc_malloc(size))) {
                return ENOMEM;
            } else {
                return EXIT_SUCCESS;
            }
...
#ifdef _ENABLE_CUDA_
static int errno_from_cuda_driver(CUresult e)
{
    switch (e) {
        case CUDA_SUCCESS:
            return 0;
        ...
        case CUDA_ERROR_CONTEXT_ALREADY_IN_USE:
            return EBUSY;
        default:
            return EIO; /* generic failure */
    }
}
#endif

#if defined(_ENABLE_CUDA_)
#define CUDA_PRINT_AND_RET_ERROR(stmt)                                         \
    do {                                                                       \
        int _errno = (stmt);                                                   \
        if (CUDA_SUCCESS != _errno) {                                          \
            fprintf(stderr, "[%s:%d] CUDA call '%s' failed with %d: %s \n",    \
                    __FILE__, __LINE__, #stmt, _errno,                         \
                    cudaGetErrorString(_errno));                               \
            return errno_from_cuda_driver(_errno);                             \
        }                                                                      \
    } while (0)
#endif
...
int init_accel(void)
{
    ...
    switch (options.accel) {
        ...
        case CUDA:
            ...
            CUDA_PRINT_AND_RET_ERROR(cudaSetDevice(dev_id));
            CUDA_PRINT_AND_RET_ERROR(cuInit(0));
            CUDA_PRINT_AND_RET_ERROR(cuDeviceGet(&cuDevice, dev_id));
            CUDA_PRINT_AND_RET_ERROR(
