 ...
 *
 * CHANGES ADDED:
 *  1) Robust fd->path resolution via /proc/self/fd/<fd>
 *  2) Autotag unknown mmap() fds by inspecting /proc/self/fd/<fd>
 *  3) Handle openat(dirfd=/dev/shm, path="relative") (common pattern)
 *  4) Make fcntl() interposer safe (avoid consuming non-existent varargs)
 */
...
static bool basename_is_target_buffer(const char *bn) {
    // Tightest safe target based on your logs:
    // OpenMPI vader shared segment files.
    return (bn && strncmp(bn, "vader_segment", 12) == 0);
}
...
/* ---------------- NEW: /proc/self/fd resolution ---------------- */
static int fd_to_path(int fd, char *buf, size_t n) {
    if (!buf || n == 0) { errno = EINVAL; return -1; }
    char linkpath[64];
    int m = snprintf(linkpath, sizeof(linkpath), "/proc/self/fd/%d", fd);
    if (m <= 0 || (size_t)m >= sizeof(linkpath)) { errno = EINVAL; return -1; }
    ssize_t r = readlink(linkpath, buf, n-1);
    if (r < 0) return -1;
    buf[r] = '\0';
    return 0;
}
...
/* ---------------- NEW: autotag from /proc/self/fd ---------------- */
static void maybe_autotag_fd_from_procfs(int fd) {
    char p[PATH_MAX];
    if (fd_to_path(fd, p, sizeof(p)) != 0) return;

    // /dev/shm or memfd cases (keep as-is if you still want them)
    if (strncmp(p, "/dev/shm/", 9) == 0) {
        char *name = p + 8; // "/ompi..."
        char *del = strstr(name, " (deleted)");
        if (del) *del = '\0';
        rec_tag_fd(fd, name);
        slog2("autotag from /proc/self/fd -> /dev/shm", name);
        return;
    }

    if (strncmp(p, "memfd:", 6) == 0) {
        const char *nm = p + 6;
        const char *del = strstr(nm, " (deleted)");
        size_t len = del ? (size_t)(del - nm) : strlen(nm);

        char pseudo[NAME_MAX];
        if (len > NAME_MAX - 2) len = NAME_MAX - 2;
        pseudo[0] = '/';
        memcpy(pseudo + 1, nm, len);
        pseudo[len + 1] = '\0';

        rec_tag_fd(fd, pseudo);
        slog2("autotag from /proc/self/fd -> memfd", pseudo);
        return;
    }

    // NEW: OpenMPI tmpdir backing files — only tag if basename is vader_segment*
    const char *bn = path_basename(p);
    if (!basename_is_target_buffer(bn)) return;

    char pseudo[NAME_MAX];
    snprintf(pseudo, sizeof(pseudo), "/%s", bn);

    // This will succeed only if your whitelist includes "/vader_segment"
    rec_tag_fd(fd, pseudo);
    if (name_whitelisted(pseudo)) {
        slog2("autotag from /proc/self/fd -> vader", pseudo);
    }
}

/* ---------------- NEW: detect if a dirfd is /dev/shm ---------------- */
static bool dirfd_is_devshm(int dirfd) {
    if (dirfd < 0) return false;
    char p[PATH_MAX];
    if (fd_to_path(dirfd, p, sizeof(p)) != 0) return false;
    return (strcmp(p, "/dev/shm") == 0) || (strncmp(p, "/dev/shm/", 9) == 0);
}
...
int openat(int dirfd, const char* path, int flags, ...){
    ...
    if(fd>=0){
        if (looks_like_devshm(path)) {
            const char* name = path + 8;
            if(name && *name) { rec_tag_fd(fd, name); slog2("openat(/dev/shm) (recorded)", name); }
        } else if (path && path[0] != '/' && dirfd_is_devshm(dirfd)) {
            // NEW: relative path under /dev/shm (openat(dirfd, "ompi...", ...))
            char namebuf[NAME_MAX];
            snprintf(namebuf, sizeof(namebuf), "/%s", path);
            rec_tag_fd(fd, namebuf);
            slog2("openat(dirfd=/dev/shm) (recorded)", namebuf);
        }
    }
    return fd;
}
...
/*
 * NEW: safer fcntl wrapper.
 * - Track only F_DUPFD / F_DUPFD_CLOEXEC.
 * - Forward other commands without consuming varargs (prevents UB on no-arg fcntl calls).
 *   NOTE: This won't correctly forward pointer-arg fcntls; add cases if you observe them.
 */
int fcntl(int fd, int cmd, ...) {
    ...
    if (cmd == F_DUPFD || cmd == F_DUPFD_CLOEXEC) {
        ...
        va_end(ap);
        int ret = real_fcntl_fn(fd, cmd, arg);
        if (ret >= 0) rec_copy(ret, fd);
        return ret;
    }

    // safest for common no-arg commands (F_GETFD, F_GETFL, etc.)
    return real_fcntl_fn(fd, cmd);
}
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset){
    ...
    char dbg[256];
    snprintf(dbg,sizeof(dbg),"mmap called len=%zu fd=%d in_range=%d shared=%d off=%lld\n",
             length, fd, in_range, shared, (long long)offset);
    slog(dbg);

    // Optional: show what the fd actually is (super useful for debugging)
    if (in_range && shared && fd >= 0) {
        char p[PATH_MAX];
        if (fd_to_path(fd, p, sizeof(p)) == 0) {
            snprintf(dbg, sizeof(dbg), "[mmap-hook/fsdax] fd=%d -> %s\n", fd, p);
            slog(dbg);
        }
    }
    ...
    if(name_copy[0]=='\0'){
        // NEW: try to infer & tag from /proc/self/fd/<fd>
        maybe_autotag_fd_from_procfs(fd);

        pthread_mutex_lock(&g_mu);
        fdrec_t* r2=rec_by_fd_nl(fd);
        if(r2 && r2->name[0]) strncpy(name_copy,r2->name,sizeof(name_copy)-1);
        pthread_mutex_unlock(&g_mu);

        if(name_copy[0]=='\0'){
            slog("mmap skipping redirect (unknown fd even after /proc/self/fd)\n");
